...
from dataset_CervicalCancer_KI import load_Trans
...
parser.add_argument('--exp-dir', default='experiment_final/PG_10', type=str,
...
parser.add_argument('--prot_start', default=10, type=int,
...
def main():
    ...
    # args.exp_dir = 'PICO-origin/experiment/PiCO-CIFAR-FINAL/cls_ds_cifarmil_pr_0.2_lr_0.02_ep_200_ps_50_lw_0.5_pm_0.99'
...
def train_classifier_pretrain(train_loader, model, loss_fn, loss_cont_fn, optimizer, epoch, args, tb_logger,
    ...
    for i, (images_w, images_s, labels, point, index) in enumerate(
        ...
        labels = labels[1]  #
        X_w, X_s, Y, index = images_w.cuda(), images_s.cuda(), labels.cuda(), index.cuda()  # 只需要labels
...
def train(train_loader, model, loss_fn, loss_cont_fn, optimizer, epoch, args, tb_logger, start_upd_prot=False,
    ...
    for i, (images_w, images_s, labels, point, index) in enumerate(
        ...
        images_w, images_s, Y, index = images_w.cuda(), images_s.cuda(), labels.cuda(), index.cuda()  # 只需要labels
        Y_true = true_labels.long().detach().cuda() 
        # point = point.cuda()
        ...
        # max_scores, pseudo_labels = torch.max(predicted_scores, dim=1) ## 选伪标签里面最大的
        # (b,)
        pseudo_target_max, pseudo_target_cont = torch.max(pseudo_score_cont, dim=1)  # 8194,
        pseudo_target_cont = pseudo_target_cont.contiguous().view(-1, 1)  # 8194,1 需要一块连续的内存
...
def train_bag(train_loader, model, loss_fn, optimizer, epoch, args):
    ...
    for i, (images, labels, index) in enumerate(
        ...
        # Y : N
        data_time.update(time.time() - end)
        images, Y, index = images[0].cuda(), labels.long().cuda(), index.cuda()  # 只需要labels
        # Y_true = true_labels.long().detach().cuda() 
        #
