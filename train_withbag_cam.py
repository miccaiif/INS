...
from dataset_CAMELYON16 import load_CAM16_end

# SEED = 0
# torch.manual_seed(SEED)
# torch.cuda.manual_seed(SEED)
parser = argparse.ArgumentParser(description='PyTorch implementation of ICLR 2022 Oral paper INS')
parser.add_argument('--dataset', default='cam16', type=str,
                    choices=['cifarmil', 'cam16'],
                    help='dataset name (cifar10)')
parser.add_argument('--exp-dir', default='experiment_final/CAM_end2end', type=str,
...
parser.add_argument('--wl', default=1, type=float,
                    help='ambiguity level (q)')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.dataset == 'cifarmil':
        ...
        train_sampler_cls, test_bag_loader,train_bag_loader = load_CAM16_end(batch_size=args.batch_size) 
    else:
    ...
    test(model, test_loader, test_bag_loader, args, epoch, logger)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        acc_test, auc_test, bag_auc, auc_bag_ins, auc_bag_ins2, auc_bag_ins3 = test(model, test_loader, test_bag_loader, args, epoch, logger)
        mmc = loss_fn.confidence.max(dim=1)[0].mean()

        with open(os.path.join(args.exp_dir, 'result.log'), 'a+') as f:
            f.write('Epoch {}: Acc {}, Best Acc {}, AUC {}, Best AUC {}, Bag AUC {},Bag AUC2 {},Bag AUC3 {}.Bag AUC4 {}. (lr {}, MMC {})\n'.format(epoch
                                                                              , acc_test, best_acc, auc_test, best_auc, bag_auc, auc_bag_ins, auc_bag_ins2, auc_bag_ins3,
...
def test(model, test_loader, test_bag_loader, args, epoch, tb_logger):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (images, labels, index) in enumerate(test_bag_loader):
            ...
            images = torch.squeeze(images,0) # 1*3*32*32
        ...
        for batch_idx, (images, images_2, each_label, labels, point,index) in enumerate(test_loader):
            # print(labels,index)
            labels = labels[0]
        ...
        print('Auc is ', auc,' Auc_bag is ', auc_bag,' Auc_bag_ins2 ',auc_bag_ins2,' Auc_bag_ins3 ',auc_bag_ins3)
    ...
    return acc_tensors[0], auc, auc_bag, 0, auc_bag_ins2, auc_bag_ins3
