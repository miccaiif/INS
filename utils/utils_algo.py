...
def generate_uniform_cv_candidate_labels(train_labels, partial_rate=0.1):
    if torch.min(train_labels) > 1:
        ...
        train_labels = train_labels - 1  ## 0~9 --> -1~8

    K = int(torch.max(train_labels) - torch.min(train_labels) + 1)  ## 10
    n = train_labels.shape[0]  ## 50000

    partialY = torch.zeros(n, K)  ## 0(50000,10)
    partialY[torch.arange(n), train_labels] = 1.0  ## set 50000 one-hot
    p_1 = partial_rate
    transition_matrix =  np.eye(K) ## (10,10) diag
    transition_matrix[np.where(~np.eye(transition_matrix.shape[0],dtype=bool))]=p_1
    print(transition_matrix) # diag=1 and others 0.1 (10x10)

    random_n = np.random.uniform(0, 1, size=(n, K)) ## (50000,10) 0~1
    ...
    for j in range(n):  # for each instance
        for jj in range(K): # for each class 
            ...
            if random_n[j, jj] < transition_matrix[train_labels[j], jj]:
                partialY[j, jj] = 1.0  ## ? 随机加噪声，有的样本的label对应多个标签1 随机的吗？？
