...
from alexnet import teacher_Attention_head, teacher_DSMIL_head
...
class INS(nn.Module):
    ...
    def __init__(self, args, base_encoder):
        ...
        self.model_teacherHead2 = teacher_DSMIL_head()
        # create the queue
    ...
    def bag_head(self, features):
        ...
        # bag_prediction,_,_,_ = self.model_teacherHead2(features)
        # print('bag_prediction:',bag_prediction.shape)
        # return bag_prediction
        return features.mean(dim=0, keepdim=True)

    def forward(self, img_q, im_k=None, partial_Y=None, args=None, eval_only=False, bag_flag=False, return_feat=False):
        if return_feat:
            
            q = self.encoder_q.encoder(img_q)  ##(64,512)
            # q = nn.Linear(torch.flatten(q).shape[0],512)
            return q # output: 512
        if bag_flag:
            # self.encoder_q.encoder = self.mlp #
            # print(img_q.shape,self.encoder_q.encoder)
            features = self.encoder_q.encoder(img_q) # 400*512
